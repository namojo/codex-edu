...
const tools: Tool[] = [
  ...
  { cmd: "codex",  label: "codex (Codex CLI)", required: true  },
];
...
// codex login
try {
  await $`codex login status`.quiet();
  rows.push([`${G}✅${N}`, "codex login", "logged in"]);
} catch {
  rows.push([`${Y}⚠️ ${N}`, "codex login", `${Y}not logged in — run: codex login${N}`]);
}
