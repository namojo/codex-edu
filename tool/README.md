...
> Python 40여 줄짜리 MCP 서버를 하나 만들어 **Codex에 등록**하고, 표준 서버라
> 다른 MCP 클라이언트에서도 그대로 재사용할 수 있는 사내 공용 계산기로 만들어 봅니다.
> 이 실습이 끝나면 "에이전트가 못 하는 일은 도구를 붙여 준다"라는 감각이 생깁니다.
...
우리가 만들 Tool은 **MCP(Model Context Protocol)** 서버입니다. MCP는 특정 제품에
묶이지 않은 **오픈 표준**이라, 이 서버 하나를 만들어 두면:

- **Codex**가 등록해 호출할 수 있고,
- MCP를 지원하는 **다른 클라이언트**(IDE 확장, 사내 에이전트 플랫폼 등)도 각자
  등록만 하면 **코드 수정 없이** 그대로 씁니다.

즉 "품질관리팀 OEE 계산기"를 클라이언트마다 다시 만들 필요가 없습니다. 이것이
사내 툴을 **자산**으로 만드는 방법입니다 — `xlsx/`의 표준용어집, `trend-report/`의
에이전트 정의와 같은 "프롬프트 밖으로 꺼내 파일로 굳힌다"(원리 2 — 지식의 외부화)의
도구 버전입니다.
...
└── register-codex.md          ← Codex에 등록하기 (상세 절차)
```
...
자세한 절차와 오류 해결은 [`register-codex.md`](register-codex.md). 요약하면:

```bash
# Codex는 실행 위치가 달라질 수 있으니 절대경로를 권장합니다.
codex mcp add production-metrics -- python3 /절대경로/codex-edu/tool/mcp-server/server.py
codex mcp list          # 등록 확인
```

등록 정보는 `~/.codex/config.toml`의 `[mcp_servers]` 절에 저장됩니다 — 이 블록을
팀 문서에 공유하면 누구나 같은 도구 환경을 재현할 수 있습니다.

---

## 5. 실습 C — 대화형과 자동화, 양쪽에서 써 보기

**대화형** — `codex`를 실행한 뒤 입력합니다:
...
**자동화** — `codex exec`는 사람이 없으므로 승인 대신 **샌드박스**로 통제합니다:
...
**관찰 포인트**: 등록은 한 번인데 대화형·자동화 양쪽에서 같은 도구가 동작합니다.
스킬 실습에서 "지식을 파일로 굳혀 어느 세션에서든 재사용한다"라고 배웠던 것의 도구
버전입니다(원리 2). 단, 스킬은 문서를 *복사*해 퍼뜨렸지만 Tool은 **서버 하나를 각
클라이언트가 등록**해 공유합니다 — 계산 로직의 원본이 한 곳뿐이라 유지보수가 더 쉽습니다.
그리고 이 도구는 CSV를 읽기만 하므로 `--sandbox read-only`, 자동화의 가장 안전한
기본값으로 충분합니다.
...
**③ 오픈 표준의 가치 = 한 번 만들어 여러 곳에서.** server.py는 Codex 전용 코드가
아닙니다 — MCP 표준 서버라서, MCP를 지원하는 클라이언트가 늘어도 도구는 그대로이고
각 클라이언트가 등록만 하면 됩니다. 이것이 "사내 도구를 자산화한다"의 실체입니다.
...
4. **원격(HTTP) MCP**: 지금은 `python3 server.py`를 로컬에서 spawn하는 stdio 방식입니다.
   팀 전체가 공유하려면 서버를 HTTP 트랜스포트로 띄우고, 각 클라이언트가 원격 주소로
   등록합니다(Codex라면 `~/.codex/config.toml`의 `[mcp_servers.*]` 항목에 원격 서버
   정보를 추가). 로컬 계산기가 **사내 API**로 승격되는 단계입니다.
