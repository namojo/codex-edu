...
Log in with your ChatGPT account (a browser window opens for authentication):
...
The script automatically retries with npm fallback if bun fails. If issues persist, check `bun pm ls -g` and add the bin dir to PATH. On macOS you can also install via Homebrew:
```bash
brew install codex
```
