...
if installed codex; then
  printf "${GREEN}✅${NC}  codex ${DIM}(already installed)${NC}\n"
else
  # 1차 bun, 실패 시 npm 폴백. macOS는 brew formula(codex)도 최종 폴백으로 병기.
  run_step "Install Codex CLI" bun install -g @openai/codex \
    || run_step "Install Codex CLI (npm fallback)" npm install -g @openai/codex \
    || run_step "Install Codex CLI (brew fallback)" brew install codex
fi
printf "     ${DIM}Login: run ${NC}${CYAN}codex login${NC}${DIM} (ChatGPT account) — verify with ${NC}${CYAN}codex login status${NC}\n"
...
if [[ ${#ERRORS[@]} -eq 0 ]]; then
  ...
  printf "\n  Next → ${CYAN}codex login${NC} (ChatGPT account), then ${CYAN}bun setup-common.ts${NC}\n"
else
