...
if ((-not $Force) -and (Installed codex)) {
    ...
    # 1차 bun, 실패 시 npm 폴백. winget 패키지는 전제하지 않는다.
    $codexInstalled = RunStep "Install Codex CLI" { bun install -g @openai/codex }
    if (-not $codexInstalled) {
        RunStep "Install Codex CLI (npm fallback)" { npm install -g @openai/codex }
        RefreshEnv
    }
}
Write-Host "     Login: run 'codex login' (ChatGPT account) — verify with 'codex login status'" -ForegroundColor DarkGray
...
if ($Errors.Count -eq 0) {
    ...
    Write-Host ""
    Write-Host "  Next → codex login (ChatGPT account), then: bun setup-common.ts" -ForegroundColor Cyan
} else {
